 ...
 */
class SpotlightRunner(private val serverLocation: String) {
//    val process: Process
    ...
    init {
        ...
        // run server
        processBuilder = ProcessBuilder("java", "-Xmx120G", "-Dthreads.min=120", "-Dthreads.max=120", "-Dthreads.core=120", "-Dpost.size.max=10485760",  "-jar", "$serverLocation/spotlight.jar",
    ...
    fun beginDownloads() {
        val serverLoc = File(serverLocation)
                .apply { if (!exists()) mkdir() } // create directory if it doesn't already exist
        ...
        val spotlightJar = File(spotLoc).apply {
            if (!exists()) {
                downloadFromUrl("http://downloads.dbpedia-spotlight.org/spotlight/dbpedia-spotlight-0.7.1.jar",
                        spotLoc)
            }
        }
        ...
        if (!modelFile.exists() || modelFile.list().isEmpty()) {
            modelFile.mkdir()
            val archive = File(compressed_loc).apply {
                if (!(exists())) {
                    downloadFromUrl("http://downloads.dbpedia-spotlight.org/2016-04/en/model/en.tar.gz",
                            compressed_loc)
                }
            }
